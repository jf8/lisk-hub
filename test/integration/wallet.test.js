  ...
  for (let i = 0; i < 6; i += 1) {
    ...
    describe('transactions', () => {
      ...
      describe('Scenario: should allow to filter transactions', () => {
        ...
        step('When I click on the "Outgoing" filter', () => {
          requestToActivePeerStub.withArgs(match.any, 'transactions', match({ senderId: accounts.genesis.address }))
            .returnsPromise().resolves({ transactions: generateTransactions(5), count: 1000 });
          clickStep('filter out');
        });
